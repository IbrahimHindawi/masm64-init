...
%rc_directory%\rc.exe" ..\resource\resource.rc
move ..\resource\resource.res .\

%ml_directory%\ml64.exe" /c /Cp /Cx /Fm /FR /W2 /Zd /Zf /Zi ..\source\%project_name%.asm > errors.txt

%link_directory%\link.exe" %project_name%.obj resource.res /debug:full /opt:ref /opt:noicf /largeaddressaware:no /def:..\source\%project_name%.def /entry:Startup /machine:x64 /map /out:%project_name%.exe /PDB:%project_name%.pdb /subsystem:windows,6.0 
type errors.txt
